    ...
    return (
        <>
            {!data[0]?.url
                ...
                : <>
                    <SwiperEl data={data} onClick={openDiv} className={{ swiper: "h-[45vh] lg:h-[30vh] max-w-[100vw] lg:max-w-[60vw]", container: "w-full p-4 relative z-0" }} />
                    <div className={`w-screen h-screen absolute top-0 left-0 flex flex-col justify-center items-center ${open ? "visible" : "invisible"}`}>
                        ...
                        <SwiperEl data={data} onClick={() => null} className={{ swiper: "h-[55vh] lg:h-[60vh]", container: `w-[90vw] p-4 z-20 relative flex flex-col items-center ${open ? "visible opacity-100" : "invisible opacity-0"} transition-opacity duration-100 ease-in-out` }} />
                    </div>
                </>}
        </>
    )
